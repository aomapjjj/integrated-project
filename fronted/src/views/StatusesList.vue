  ...
  <div>
    ...
    <dialog id="my_modal_4" class="itbkk-modal-status modal">
      <div class="modal-box w-full md:w-11/12 max-w-5xl mx-auto">
          ...
          <div class="flex justify-end">
            <form form @submit.prevent="submitForm" method="dialog">
              <button type="submit" class="itbkk-button-confirm btn mr-2"
                style="flex: 3; margin: 10px; background-color: #f785b1" :disabled="!isFormValid">
                Save
              </button>
            </form>
        ...
        <!-- Add Error Alert -->
        <div role="alert" class="alert shadow-lg alert-error" v-show="notAdd" style="
            position: fixed;
            top: 20px;
            left: 50%;
            transform: translateX(-50%);
        ...
        <!-- Add Success Alert -->
        <div role="alert" class="alert shadow-lg" :class="{ hidden: !showAlertAfterAdd }" style="
            position: fixed;
            top: 20px;
            left: 50%;
            transform: translateX(-50%);
  ...
  <div class="flex flex-col items-center mt-1">
    <div class="overflow-x-auto">
      <div class="min-w-full">
        ...
        <!-- Edit Alert -->
        <div role="alert" class="alert shadow-lg alert-error" v-show="notEdit" style="
            position: fixed;
            top: 20px;
            left: 50%;
            transform: translateX(-50%);
        ...
        <table class="table-auto1 mt-5 rounded-xl overflow-hidden mb-10" style="table-layout: fixed">
          ...
          <tbody>
            <tr class="itbkk-item" v-for="(item, index) in myStatuses.getStatuses()" :key="index">
              ...
              <td class="itbkk-status-description px-4 py-2 text-center md:text-left text-sm text-gray-700" :class="{
        ...
        : item.description
              ...
              <td class="px-4 py-2 text-center md:text-left text-sm text-gray-700 itbkk-status">
                ...
                <dialog id="my_modal_edit" class="modal">
                  <div class="modal-box w-full md:w-11/12 max-w-5xl mx-auto" :key="index">
                    <span class="block text-2xl font-bold leading-6 text-gray-900 mb-1" style="margin: 15px"></span>
                      ...
                      <div class="flex justify-end">
                        <form method="dialog">
                          <button @click="UpdateStatus" type="submit" class="itbkk-button-confirm btn mr-2"
                            :class="{ disabled: !isEdited }" :disabled="!isFormValid || !isEdited">
                            Save
                          </button>
                        </form>
                ...
                <div role="alert" class="alert shadow-lg" :class="{ hidden: !showAlertAfterDelete }" style="
                    position: fixed;
                    top: 20px;
                    left: 50%;
                    transform: translateX(-50%);
                ...
                <dialog id="my_modal_deleteTrans" class="modal">
                  <div class="modal-box" style="max-width: 500px">
                    ...
                    <div class="itbkk-status mb-4 mt-2 w-full flex flex-col items-center">
                      <span class="block text-lg font-bold leading-6 text-gray-900 mb-2 text-center">Transfer a task
                        to</span>
                    ...
                    <!-- LimitAlert -->
                    <div role="alert" class="alert shadow-lg alert-error" v-show="limitAlert" style="
                          position: fixed;
                          top: 20px;
                          left: 50%;
                          transform: translateX(-50%);
