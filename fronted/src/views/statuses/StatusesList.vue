    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <div class="flex flex-col flex-1">
        ...
        <div class="mt-9 px-10 flex justify-between items-center">
          ...
          <button :disabled="disabledButtonWhileOpenPublic" class="itbkk-button-add btn ml-4" :style="{
            backgroundColor: disabledButtonWhileOpenPublic ? '#d3d3d3' : '#F785B1',
          ...
          <div v-if="showAddModal || showEditModal"
            ...
            <div class="itbkk-modal-alert modal-box w-1/3">
              <h3 class="font-bold text-lg customPurple">{{ showAddModal ? 'Add New Status' : 'Edit Status' }}</h3>
              <div class="py-4">
                <label class="block mb-2 font-bold">Status Name</label>
                <input type="text" v-model="status.name" class="input input-bordered w-full"
                  placeholder="Enter status name" />
                <p class="text-sm text-gray-400 mt-1">{{ status.name?.length }}/50</p>

                <label class="block mt-4 mb-2 font-bold">Description</label>
                <textarea v-model="status.description" class="textarea textarea-bordered w-full" rows="4"
                  placeholder="Enter description"></textarea>
                <p class="text-sm text-gray-400 mt-1">{{ status.description?.length }}/200</p>
              </div>
              <div class="modal-action flex justify-end">
                <button @click="closeSharedModal" class="btn bg-gray-500 text-white">Cancel</button>
                <button v-if="showAddModal" @click="submitForm" :disabled="!isFormValid"
                  class="btn bg-green-500 text-white ml-2">
                  Save
                </button>
                <button v-else @click="updateSharedStatus" :disabled="!isFormValid"
                  class="btn bg-green-500 text-white ml-2">
                  Update
                </button>
              </div>
          ...
          <div class="overflow-x-auto">
            <div class="min-w-full">
              ...
              <table class="table-auto1 mt-5 rounded-xl overflow-hidden mb-10" style="table-layout: fixed">
                ...
                <tbody>
                  <tr class="itbkk-item" v-for="(item, index) in myStatuses.getStatuses()" :key="index">
                    <!-- ID -->
                    ...
                    <td class="px-4 py-2 text-center md:text-left text-sm text-gray-700 itbkk-status">
                      <button :disabled="disabledButtonWhileOpenPublic" :class="[
                        'itbkk-button-edit',
                        'btn',
                        'rounded-full',
                        ...
                        cursor: disabledButtonWhileOpenPublic
                          ...
                          : 'pointer',
                        opacity: disabledButtonWhileOpenPublic ? 0.6 : 1
                      }" @click="openEditModal(item.id)" v-if="item.name !== 'No Status' && item.name !== 'Done'">
                      ...
                      <button :disabled="disabledButtonWhileOpenPublic" :class="[
                        ...
                        class="itbkk-button-delete btn bg-red-400 rounded-full" style="margin-left: 10px"
                      ...
                      <ConfirmModal :openModal="openModalDelete" @confirm="confirmDelete()"
                      ...
                      <dialog id="my_modal_deleteTrans" class="modal">
                        <div class="modal-box" style="max-width: 500px">
                          <h3 class="itbkk-message font-bold text-lg" style="color: #9391e4">
                            Transfer a Status
                          </h3>
                          ...
                          <div class="itbkk-status mb-4 mt-2 w-full flex flex-col items-center">
                            <span class="block text-lg font-bold leading-6 text-gray-900 mb-2 text-center">Transfer a
                              task to</span>
                            ...
                            <div class="flex justify-center w-full max-w-xs mt-4 space-x-5">
                              <button class="itbkk-button-cancel btn" @click="closeModalTrans" style="color: #eb4343">
                                Cancel
                              </button>
