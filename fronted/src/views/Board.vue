...
import { getBoard, addBoard, deleteItemById } from "../libs/fetchUtils.js"
...
onMounted(async () => {
  const result = await getBoard(baseUrlBoard); // result มีทั้ง status และ data

  if (result.status === 200) {
    BoardsList.value = result.data; // ถ้าสถานะเป็น 200 ให้นำข้อมูลมาใช้
  } else if (result.status === 401) {
    // กรณี unauthorized ให้ redirect ไปที่หน้า login หรือแสดงข้อความเตือน
    localStorage.removeItem("access_token")
    router.push({ name: "Login" })
  } else {
    console.log(`Error: Status code ${result.status}`); // จัดการกับ error อื่น ๆ
  }
});
            ...
            <router-link to="/board/add">
              <div
                ...
                <div class="flex flex-col items-center relative">
                  ...
                  <span class="itbkk-button-create mt-1 text-sm">Create Board</span>
                </div>
              </div>
            </router-link>
          </button>
          ...
          <div class="grid grid-cols-4 gap-6">
            <div v-for="(item, index) in BoardsList" :key="index">
              <div class="bg-white rounded-lg shadow p-4">
                <div class="relative">
                  <div class="bg-gray-100 h-32 w-full rounded-md mb-4 flex items-center justify-center"></div>
                  ...
                  <div class="dropdown absolute top-2 right-2 text-gray-500 hover:text-gray-700">
                    ...
                    <ul tabindex="0" class="dropdown-content menu bg-base-100 rounded-box z-[1] w-40 p-2 shadow">
                      <li><a>Edit</a></li>
                      <li @click="openModalToDeleteBoard(item.id)" class="customRed">
                        <a>Delete</a>
                      </li>
                    </ul>
                  </div>
            ...
            >
              ...
              {{ status.name?.length }}/50
                ...
                <div class="flex justify-center">
                  <form method="dialog">
                    <button type="submit" class="itbkk-button-ok btn flex-3 mr-2 bg-customPink" @click="submitForm()"
                      :disabled="!isValidName">
                      Save
                    </button>
                  </form>
                  <router-link to="/board">
                    <button @click="cancelAction" class="itbkk-button-cancel btn">
                      Cancel
                    </button>
                  </router-link>
                </div>
