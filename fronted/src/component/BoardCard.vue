  ...
  <div class="p-4 overflow-y-auto h-screen max-h-screen md:h-[80vh] lg:h-[75vh] xl:h-[70vh]">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <div v-for="(item, index) in getBoardsByOwner(BoardsList, userName)" :key="index">
        ...
        <div v-if="openModalToDelete && selectedItemIdToDelete === item.id"
          class="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div class="w-full max-w-lg p-3 relative mx-auto my-auto rounded-xl shadow-lg bg-white">
            <div>
              <div class="text-center p-3 flex-auto justify-center leading-6">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-16 h-16 flex items-center customRed mx-auto"
    ...
    <div class="p-3">
      <div v-for="(item, index) in getBoardsCollabsByOwner(BoardsList, userName)" :key="index">
        <ul role="list" class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
          <li class="itbkk-collab-item col-span-1 divide-y divide-gray-200 rounded-lg bg-white shadow">
            <div class="flex w-full items-center justify-between space-x-6 p-6">
              <div class="flex-1 truncate">
                <div class="flex items-center space-x-3">
                  <h3 class="itbkk-board-name truncate text-sm font-medium text-gray-900">
                    {{ item.name }}
                  </h3>
                  <span
            ...
            <div>
              <div class="-mt-px flex divide-x divide-gray-200">
                <div class="flex w-0 flex-1">
                  <span @click="toBoardsList(item.id)"
