...
const getCollaboratorAccessRight = (collaborators, name) => {
  for (let collab of collaborators) {
    if (collab.name === name + ' (Pending Invite)') {
      return collab.accessRight
    }
  }
  return false
}
...
<template>
  ...
  <div class="p-4 overflow-y-auto h-screen max-h-screen md:h-[80vh] lg:h-[75vh] xl:h-[70vh]">
    <!-- Personal Boards -->
    <slot name="labelPersonalBoard"></slot>
    <div v-if="boardsOwner?.length === 0">
      <EmptyBoard />
    </div>
    <div v-else class="px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <div v-for="(item, index) in boardsOwner" :key="index">
        <div ref="colorCard" :class="`${currentColor[item.id]}`"
          ...
          <PersonalAndCollabBoard>
            ...
            <template #colorBoard>
              <span @click="setColor('bg-gray-300', item.id)"
                class="inline-block rounded-full bg-white border-4 border-gray-400 p-1 text-xs font-bold mr-3"></span>
              <span @click="setColor('bg-purple-300', item.id)"
                class="inline-block rounded-full bg-white border-4 border-purple-400 p-1 text-xs font-bold mr-3"></span>
              <span @click="setColor('bg-indigo-300', item.id)"
                class="inline-block rounded-full bg-white border-4 border-indigo-400 p-1 text-xs font-bold mr-3"></span>
              <span @click="setColor('bg-blue-300', item.id)"
                class="inline-block rounded-full bg-white border-4 border-blue-400 p-1 text-xs font-bold mr-3"></span>
              <span @click="setColor('bg-yellow-300', item.id)"
                class="inline-block rounded-full bg-white border-4 border-yellow-400 p-1 text-xs font-bold mr-3"></span>
              <span @click="setColor('bg-red-300', item.id)"
                class="inline-block rounded-full bg-white border-4 border-red-400 p-1 text-xs font-bold mr-3"></span>
            </template>
            <template #Btn>
        ...
        <ConfirmModal :openModal="openModalToDelete && selectedItemIdToDelete === item.id" @confirm="confirmDelete()"
          ...
          <template #confirmBtn>
            <span
              class="inline-flex w-full justify-center rounded-md bg-red-500 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-300 sm:ml-3 sm:w-auto">Delete</span>
          </template>
          <template #cancelBtn>
            <span
              class="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">Cancel</span>
          </template>
        </ConfirmModal>
      </div>
    </div>
    ...
    <div v-if="boardsCollab" class="px-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <div v-for="(board, index) in boardsCollab" :key="board.id">
        <div ref="colorCard" :class="`${currentColor[board.id]}`"
          ...
          <PersonalAndCollabBoard :is-show="getCollaboratorStatus(
            ...
            <template #toggle>
              ...
              "
                class="itbkk-access-right text-white text-xs font-bold rounded-full inline-block ml-4 py-1.5 px-4 bg-gray-400 cursor-pointer">
                {{
                  getCollaboratorAccessRight(
                    boardsCollab[index].collaborators,
                    userName
                  )
                }}
            ...
            <template #name>
              <div
                ...
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-customPink" viewBox="0 0 24 24">
                  <path
            ...
            <template #ownerName>
              <div class="flex items-center space-x-2 cursor-pointer hover:text-gray-900 transition duration-100 mx-4">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="w-5 h-5 text-customPink">
                  <path fill="currentColor"
            ...
            <template #pendingBtn>
              <button class="btn rounded-full customBgYellow" @click="toBoardsInvitations(board)">
                Accept/Decline
              </button>
            </template>
            <template #Btn>
              <button class="btn rounded-full customBgYellow" @click="toBoardsList(board)">
                View
              </button>
              <button class="itbkk-leave-board btn bg-red-400 rounded-full" @click="showRemoveModal(board.id)">
                Leave
              </button>
            </template>
          </PersonalAndCollabBoard>
  ...
  <ConfirmModal :openModal="showConfirmModal" @confirm="confirmRemove()" @cancel="showConfirmModal = false">
    ...
    <template #confirmBtn>
      <span
        class="inline-flex w-full justify-center rounded-md bg-red-500 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-300 sm:ml-3 sm:w-auto">Leave</span>
    </template>
    <template #cancelBtn>
      <span
        class="mt-3 inline-flex w-full justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto">Cancel</span>
